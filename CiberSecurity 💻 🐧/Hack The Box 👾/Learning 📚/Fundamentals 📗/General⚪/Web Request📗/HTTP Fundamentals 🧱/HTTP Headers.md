We can divide headers into following categories:
1. `General Headers`
2. `Entity Headers`
3. `Request Headers`
4. `Response Headers`
...
# General Headers
This one are used in both HTTP requests en responses. ==They describe the message rather than its contents==
...
# Entity Headers
Similar to general headers. This one can be common to both the request and response. ==They are used to describe the content transferred by message==
...
# cURL
Some headers labels, like `User-Agent` or `Cookie`, have their own flags. For example, we can use the `-A` to set our `User-Agent`
```
┌──(kali㉿kali)-[~]
└─$ curl https://inlanefreight.com -A 'Mozilla/5.0 (Linux; Android 16; Pixel 10) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/152.0.0.0 Mobile Safari/537.36' -v
```